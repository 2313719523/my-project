...
export default {
  ...
  methods: {
    // 根据可用宽度计算显示的菜单数量（以顶部导航实际容器为准）
    setVisibleNumber() {
      let containerWidth = 0
      // this.$el 是 el-menu，父节点就是 navbar 中的 center-section
      if (this.$el && this.$el.parentElement) {
        containerWidth = this.$el.parentElement.getBoundingClientRect().width
      } else {
        // 兜底：实在取不到就用 body 宽度
        containerWidth = document.body.getBoundingClientRect().width
      }
      // 预估单个菜单项宽度（包含左右间距），避免太挤
      const itemWidth = 110
      const count = Math.floor(containerWidth / itemWidth)
      this.visibleNumber = Math.max(1, count)
    },
    // 菜单选择事件
